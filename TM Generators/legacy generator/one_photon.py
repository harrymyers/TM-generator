...
mass_muon = 105.5  # MeV - mass of muon for new scattering angle calculation
x_min = M_TM / E_beam
N_events = 1000000
Lambda = 20.0  # MeV
...
# NEW Angular spread calculation
theta0 = mass_muon / E_beam  # New scattering angle calculation
...
# Print the new theta0 value for comparison
print(f"New theta0 = {theta0:.6f} radians = {theta0*1000:.3f} mrad")
print(f"Old theta0 would be Lambda/(x*E_beam) ≈ {Lambda/(x_min*E_beam):.6f} radians = {Lambda/(x_min*E_beam)*1000:.3f} mrad")

# Angular correlation
plt.figure(figsize=(7,6))
plt.hist2d(theta_y_eplus_mrad, theta_y_eminus_mrad, bins=100, range=[[0,200],[0,200]], cmap='viridis')
plt.colorbar(label='Counts')
plt.title("Angular Correlation between e⁺ and e⁻ (New Scattering)")
plt.xlabel("e⁺ vertical angle (mrad)")
plt.ylabel("e⁻ vertical angle (mrad)")
plt.grid(True)
plt.tight_layout()
plt.savefig("fig1_angular_correlation_new.png")
plt.close()

# Zoomed-in angular correlation (40-60 mrad region)
plt.figure(figsize=(7,6))
plt.hist2d(theta_y_eplus_mrad, theta_y_eminus_mrad, bins=100, range=[[40,60],[40,60]], cmap='plasma')
plt.colorbar(label='Counts')
plt.title("Angular Correlation Zoom (40–60 mrad) - New Scattering")
plt.xlabel("e⁺ vertical angle (mrad)")
plt.ylabel("e⁻ vertical angle (mrad)")
plt.grid(True)
plt.tight_layout()
plt.savefig("fig2_zoomed_acceptance_new.png")
plt.close()

# Momentum space
plt.figure(figsize=(8,6))
plt.hist2d(p_eplus[:,1], p_eplus[:,2], bins=200,range=[[-120,120],[0,800]], cmap='Blues')
plt.colorbar(label='Counts')
plt.xlabel("p_y (MeV/c)")
plt.ylabel("p_z (MeV/c)")
plt.title("e⁺ Momentum: Transverse vs Longitudinal (New Scattering)")
plt.tight_layout()
plt.savefig("fig3_eplus_momentum_zoom_new.png", dpi=300)
plt.close()

plt.figure(figsize=(8,6))
plt.hist2d(p_eplus[:,1], p_eplus[:,2], bins=200, cmap='Blues')
plt.colorbar(label='Counts')
plt.xlabel("p_y (MeV/c)")
plt.ylabel("p_z (MeV/c)")
plt.title("e⁺ Momentum: Transverse vs Longitudinal (New Scattering)")
plt.tight_layout()
plt.savefig("fig3_eplus_momentum_new.png", dpi=300)
plt.close()

plt.figure(figsize=(8,6)) 
plt.hist2d(p_eminus[:,1], p_eminus[:,2], bins=200,range=[[-120,120],[0,800]], cmap='Reds')
plt.colorbar(label='Counts')
plt.xlabel("p_y (MeV/c)")
plt.ylabel("p_z (MeV/c)")
plt.title("e⁻ Momentum: Transverse vs Longitudinal (New Scattering)")
plt.tight_layout()
plt.savefig("fig4_eminus_momentum_new.png", dpi=300)
plt.close()

plt.figure(figsize=(8,6))
plt.hist2d(p_eminus[:,1], p_eminus[:,2], bins=200, cmap='Reds')
plt.colorbar(label='Counts')
plt.xlabel("p_y (MeV/c)")
plt.ylabel("p_z (MeV/c)")
plt.title("e⁻ Momentum: Transverse vs Longitudinal (New Scattering)")
plt.tight_layout()
plt.savefig("fig4_eminus_zoom_momentum_new.png", dpi=300)
plt.close()

# Opening angle
plt.figure(figsize=(8,5))
plt.hist(opening_angle_mrad, bins=100, color='seagreen')
plt.title("Opening Angle Distribution (Lab Frame) - New Scattering")
plt.xlabel("Opening angle (mrad)")
plt.tight_layout()
plt.savefig("fig5_opening_angle_mrad_new.png")
plt.close()

plt.figure(figsize=(8,5))
plt.hist(opening_angle_mrad, bins=100, range=(100,500), color='darkorange')
plt.title("Opening Angle (100 - 500 mrad) - New Scattering")
plt.xlabel("Opening angle (mrad)")
plt.tight_layout()
plt.savefig("fig6_opening_angle_zoomed_new.png")
plt.close()

# Decay length
plt.figure(figsize=(8,5))
plt.hist(decay_lengths_mm, bins=100, color='slateblue', log=True)
plt.title("Decay Length Distribution - New Scattering")
plt.xlabel("Decay length (mm)")
plt.tight_layout()
plt.savefig("fig7_decay_length_distribution_new.png")
plt.close()
...
plt.xlabel("x (mm)")
plt.ylabel("y (mm)")
plt.title("e⁺ Detector Hit Map at z=0.5 m (New Scattering)")
plt.tight_layout()
plt.savefig("fig8_detector_hits_eplus_new.png")
plt.close()
...
plt.title("e⁻ Detector Hit Map at z=0.5 m (New Scattering)")
plt.tight_layout()
plt.savefig("fig9_detector_hits_eminus_new.png")
plt.close()
...
plt.title("Decay Length vs TM Energy - New Scattering")
plt.tight_layout()
plt.savefig("fig10_decay_vs_energy_new.png")
plt.close()
...
plt.xlabel("e⁺ vertical angle (mrad)")
plt.ylabel("Decay length (mm)")
plt.title("Decay Length vs e⁺ Vertical Angle - New Scattering")
plt.tight_layout()
plt.savefig("fig11_decay_vs_angle_new.png")
plt.close()
...
lhe_lines.append("    Generated with true muonium decay + displacement (New Scattering)")
lhe_lines.append("</header>")
...
with open("true_muonium_events_with_displacement_new_scattering.lhe", "w") as f:
    f.write("\n".join(lhe_lines))
