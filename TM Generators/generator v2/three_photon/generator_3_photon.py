...
# LHE output
WRITE_GZIP = True
OUTFILE = Path(__file__).parent / ("tm_3gamma_noaccept.lhe.gz" if WRITE_GZIP else "tm_3gamma_noaccept.lhe")
...
if MAKE_QA_PLOTS:
    ...
    plt.legend(); plt.tight_layout(); plt.savefig(Path(__file__).parent / 'qa_x_pdf.png'); plt.close()
    ...
    plt.savefig(Path(__file__).parent / 'qa_x_cdf.png'); plt.close()
    ...
    plt.savefig(Path(__file__).parent / 'qa_theta_ep_mrad.png'); plt.close()

    plt.figure(figsize=(8,5))
    plt.hist(th_em, bins=150)
    plt.xlabel('e⁻ vertical angle (mrad)'); plt.ylabel('Events / bin')
    plt.title('e⁻ vertical angle (no acceptance)'); plt.tight_layout()
    plt.savefig(Path(__file__).parent / 'qa_theta_em_mrad.png'); plt.close()

    # Angular correlation between e+ and e-
    plt.figure(figsize=(10,8))
    
    # Create 2D histogram with appropriate binning
    # Use percentiles to determine axis limits to capture important features
    ep_99 = np.percentile(th_ep, 99)
    em_99 = np.percentile(th_em, 99)
    
    # Set reasonable limits (don't go beyond 99th percentile to avoid outliers)
    max_angle = min(max(ep_99, em_99), 10000)  # Cap at 50 mrad for visibility
    
    # Create bins with good resolution
    bins = np.linspace(0, max_angle, 100)
    
    # Create 2D histogram
    H, xedges, yedges = np.histogram2d(th_ep, th_em, bins=[bins, bins])
    
    # Plot with log scale for better visibility of correlations
    # Determine a safe vmin for LogNorm to avoid log(0)
    nonzero = H[H > 0]
    vmin = nonzero.min() if nonzero.size > 0 else 1e-3
    plt.imshow(H.T, origin='lower', extent=[0, max_angle, 0, max_angle], 
               aspect='equal', cmap='viridis', norm=LogNorm(vmin=vmin, vmax=H.max() if H.max() > 0 else 1))
    
    plt.colorbar(label='Events / bin')
    plt.xlabel('e⁺ vertical angle (mrad)')
    plt.ylabel('e⁻ vertical angle (mrad)')
    plt.title('e⁺ vs e⁻ Angular Correlation (3γ production)')
    
    # Add diagonal line for reference (perfect correlation)
    plt.plot([0, max_angle], [0, max_angle], 'r--', alpha=0.7, linewidth=1, label='Perfect correlation')
    plt.legend()
    
    plt.tight_layout()
    plt.savefig(Path(__file__).parent / 'qa_3photon_angular_correlation.png', dpi=150, bbox_inches='tight')
    plt.close()
